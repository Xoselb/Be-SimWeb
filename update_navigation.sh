...
for file in "${HTML_FILES[@]}"; do
    if [ -f "$file" ]; then
        ...
        # This adds the Boutique link before the Contact link
        sed -i '' -e '/<li><a href="contacto.html">Contact</a><\/li>/i\                    <li><a href="merch.html">Boutique</a></li>' "$file"
        
        # Update active class for the current page
        # Remove active class from all links
        sed -i '' -e 's/class="active"//g' "$file"
        # Add active class to the current page
        if [[ "$file" == "merch.html" ]]; then
            sed -i '' -e 's|<a href="merch.html"|& class="active"|' "$file"
        fi
